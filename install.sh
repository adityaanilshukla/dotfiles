...
echo "    reboot."
echo "  - Idle sleep on AC ends long-running sessions: with the display off and"
echo "    no wake assertion held, the machine sleeps about a minute later, and"
echo "    Claude Code's own caffeinate is often the only thing holding it up."
echo "    Run 'pmset -c sleep 0' as root to stop that on the adapter only;"
echo "    battery still sleeps normally. Not automated here because it needs a"
echo "    password and it is a per-machine call -- a laptop shut down nightly"
echo "    does not need it. Does not survive a closed lid; that is 'awake on'."
