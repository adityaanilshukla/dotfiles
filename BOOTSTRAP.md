...
| Point **Raycast** at `raycast/` (Extensions > Script Commands > Add Directories), then disable Raycast's own **Restart** and **Shut Down** | Raycast's built-ins restart via an Apple Event with no `state saving preference`, and System Events documents omitted as *always save* — so they reopen every running app whatever the checkbox says, and never draw the dialog that carries it. `raycast/restart.sh` passes the parameter. Disable the built-ins first: these are titled plainly, so with both enabled you get two identical entries called Restart and no way to see which is which. |
...
| `raycast/` | Raycast Script Commands; add the directory in Raycast, no symlink |
